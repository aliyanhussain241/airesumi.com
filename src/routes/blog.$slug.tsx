...
function renderContent(content: string) {
  return content.split("\n\n").map((block, i) => {
    if (block.startsWith("# ")) return <h1 key={i} className="text-[28px] font-bold text-[#111827] mt-8 mb-4">{block.slice(2)}</h1>;
    if (block.startsWith("## ")) return <h2 key={i} className="text-[22px] font-bold text-[#111827] mt-7 mb-3">{block.slice(3)}</h2>;
    if (block.startsWith("### ")) return <h3 key={i} className="text-[18px] font-bold text-[#111827] mt-5 mb-2">{block.slice(4)}</h3>;
    if (block.startsWith("- ") || block.startsWith("* ")) {
      const items = block.split("\n").filter(l => l.startsWith("- ") || l.startsWith("* "));
      return <ul key={i} className="list-disc list-inside space-y-2 my-4 text-[16px] text-[#374151] leading-relaxed">{items.map((item, j) => <li key={j}>{item.slice(2)}</li>)}</ul>;
    }
    if (block.startsWith("> ")) return <blockquote key={i} className="border-l-4 border-[#EA580C] pl-5 py-2 my-4 bg-orange-50/50 rounded-r-xl"><p className="text-[16px] text-[#374151] italic">{block.slice(2)}</p></blockquote>;
    if (!block.trim()) return null;
    const html = block
      .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
      .replace(/\*(.+?)\*/g, '<em>$1</em>')
      .replace(/`(.+?)`/g, '<code class="bg-orange-50 text-orange-700 px-1.5 py-0.5 rounded text-[14px]">$1</code>');
    return <p key={i} className="text-[16px] text-[#374151] leading-[1.85] mb-4" dangerouslySetInnerHTML={{ __html: html }} />;
  }).filter(Boolean);
}
    ...
    <div className="min-h-screen pt-[68px]">
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <motion.article initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h1 className="text-[34px] font-bold text-[#111827] leading-tight tracking-tight mb-4">
            {post.title}
          </h1>
          ...
          {tags.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap mt-8 pt-6 border-t border-white/60">
              ...
              {tags.map(tag => (
                <span key={tag} className="text-[12px] bg-gray-100/80 text-gray-600 px-3 py-1 rounded-full">{tag}</span>
              ))}
            </div>
          )}
        </motion.article>
        ...
        <div className="mt-10 bg-gradient-to-br from-orange-500 to-orange-600 rounded-3xl p-7 text-white text-center">
          ...
          <Link to="/resume" className="inline-flex items-center gap-2 bg-white text-[#EA580C] font-bold px-6 py-3 rounded-2xl hover:bg-orange-50 transition-colors no-underline text-[14px]">
